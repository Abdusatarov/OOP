...
class StudentManagementSystem:
    ...
    def __init__(self, institution_name: str = "University",
                 db_file: str = "students.json"):
        self._institution: str = institution_name
        self._students: list[Student] = []     # private registry
        self._db_file: str = db_file           # путь к JSON файлу

        # Автозагрузка при старте — если файл есть, читаем его
        self._load()
    ...
    def _student_to_dict(self, s: "Student") -> dict:
        """
        Конвертирует объект Student в словарь для JSON.
        Инкапсуляция: работаем только через публичный интерфейс (@property).
        Пример JSON одного студента:
          {
            "student_id": 1001,
            "name": "Aidana Bekova",
            "email": "aidana@sdu.edu.kz",
            "major": "Computer Science",
            "year": 2,
            "grades": {"OOP": 95.0, "Algorithms": 88.5},
            "registered_at": "2026-04-20 12:00"
          }
        """
        return {
            ...
            "major":         s.major,
            "year":          s.year,
            "grades":        s.grades,          # копия через @property
            "registered_at": s._registered_at,
        }

    def _dict_to_student(self, data: dict) -> "Student":
        """
        Восстанавливает объект Student из словаря JSON.
        Шаги:
          1. Создаём новый Student (получает новый auto-ID)
          2. Принудительно заменяем ID на сохранённый
          3. Восстанавливаем оценки и дату напрямую
        """
        student = Student(
            name=data["name"],
            email=data["email"],
        ...
        # Перезаписываем auto-ID на сохранённый из файла
        student._Student__student_id = data["student_id"]
        # Восстанавливаем оценки в приватный словарь
        student._grades = {k: float(v) for k, v in data.get("grades", {}).items()}
        # Восстанавливаем дату регистрации
        student._registered_at = data.get("registered_at", "—")
        return student

    def save(self):
        """
        Сохраняет всю базу студентов в JSON файл.
        Формат файла:
          {
        ...
        Вызывается автоматически после каждого изменения данных.
        """
        payload = {
            "institution": self._institution,
            "id_counter":  Student._id_counter,   # сохраняем счётчик ID
            "students":    [self._student_to_dict(s) for s in self._students],
        }
        with open(self._db_file, "w", encoding="utf-8") as f:
            json.dump(payload, f, ensure_ascii=False, indent=2)
        print(f"  💾  Сохранено в {self._db_file} ({len(self._students)} студентов)")

    def _load(self):
        """
        Загружает студентов из JSON файла при запуске программы.
        Если файла нет — это нормально (первый запуск), начинаем с пустой базы.
        Если файл повреждён — выводим предупреждение и начинаем заново.
        """
        if not os.path.exists(self._db_file):
            return   # первый запуск — файла ещё нет, это нормально

        try:
            with open(self._db_file, "r", encoding="utf-8") as f:
                payload = json.load(f)   # dict из JSON

            # Восстанавливаем глобальный счётчик ID чтобы новые ID не повторялись
            Student._id_counter = payload.get("id_counter", 1000)

            # Восстанавливаем каждого студента из его словаря
            for data in payload.get("students", []):
                student = self._dict_to_student(data)
                self._students.append(student)

            if self._students:
                print(f"  💾  Загружено {len(self._students)} студентов из {self._db_file}")

        except (json.JSONDecodeError, KeyError) as e:
            print(f"  ⚠️  Ошибка чтения {self._db_file}: {e}. Начинаем с пустой базы.")
    ...
    def register_student(self, name: str, email: str,
        ...
        self.save()          # автосохранение в students.json
        return student
